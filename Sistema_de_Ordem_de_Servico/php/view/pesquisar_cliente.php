<?php  
	...
	function pesquisar(){
		if(isset($_GET['input-pesquisar-cliente'])){
			echo "
			...
			$sql = new Sql();
			$resultadoCliente = $sql->select("select * from cliente where sexo like :busca or nome like :busca or cpf like :busca or dataNascimento like :busca", array(
				":busca"=>"%".$_GET['input-pesquisar-cliente']."%"
			));
				...
				foreach ($cliente as $campo => $valor) {
					...
					if($campo == 'dataNascimento'){
						...
						echo "<div class='coluna col1'>
								<input type='button' class='botao-cadastro' onclick='editarEntidade($idCliente)' value='&#9998;'>
							</div>";

						echo "<div class='coluna col1 sem-padding-right'>
								<input type='button' class='botao-cadastro' onclick='excluirEntidade($idCliente)' value='&#10005;'>
							</div>";
...
<html>
	...
	<body>
		...
		<!-- Conteúdo do Sistema -->
		<div class="sessao" id="pesquisar-cliente">
			<div class="linha">
				...
				<form action="" method="get">
					...
					<div class="coluna col8">
						<input type="text" name="input-pesquisar-cliente" id="input-pesquisar-cliente" placeholder="Pesquisar" required>
					</div>							
				...
				<div class="coluna col2">
					<input type="submit" value="Cadastrar" class="botao-cadastro" onclick="encaminharPagina('cadastrar_cliente.php')">
				</div>
				<?php  					
					pesquisar();					
				?>
